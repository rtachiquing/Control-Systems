...
Vc(s)            1
----- = -----------------------
Vi(s)    1/2*s**2  + 3/2*s + 1

Respuesta a escalón unitario

Vc(s)            1                   1                1
----- = ----------------------- *  ---- =  --------------------------
Vi(s)    1/2*s**2  + 3/2*s + 1       s      1/2*s**3  + 3/2*s**2 + s
...
f1 = r[0]*sympy.exp(p[0]*t) + r[1]*sympy.exp(p[1]*t) + r[2]*sympy.exp(p[2]*t)
print(f1)

p = sympy.plotting.plot(f, f1, (t,0,10))
p[1].line_color = 'red'
p.show()
